...
- **The run did not cover everything.** Ctrl-C finishes the current scenario, cleans up and marks the run `interrupted`. Under `--only`, the scenarios nobody executed keep their earlier run id and their rows read `PASS (earlier)`. Either way the report says so, naming the run.
