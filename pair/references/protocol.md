...
The tester does not wait for the implementation to finish: scenarios, test code and reproductions are written from the plan and the agreed interfaces, in the tester's own files, and partial results go out as `NOTE`s. A test written before the code is red by design; that is a `NOTE`, not a `BLOCK`. Nor does the tester wait for the plan's `OK`: while the plan is under review, scenarios and isolated test drafts may be written in new files, marked provisional in the `NOTE` that mentions them. What waits for the approved plan is any edit to a file that already exists or that another member may own.
