...
- **Where a task carries an `Issue:` segment**, make its id cell a link to that issue — `[APP-15](https://github.com/…/issues/42)`. The URL is already in the plan text, so this costs no column, no width and no network call. Where no task carries one, nothing changes: the same discipline as dropping the `P` column when nothing carries a priority token. What the table reports is the plan's belief about that issue, not GitHub's — this skill never asks GitHub, and the plan is the source anyway. `project-docs:docs-sync` is what reconciles the two.
