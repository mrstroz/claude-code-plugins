...
`qa/skills/ui-test-report` (0.1.0) is the first skill here that depends on an MCP server rather than a bundled client, and the first whose deliverable is a directory of images. It was extracted from a real 41-scenario run against a filter redesign, and the decisions that came out of that run are the plugin:
