...
6. **The GitHub issue**, if `docs/docs.config.json` enables it. Most trees have no such file and this step does not exist for them — skip it in silence rather than mentioning GitHub. Where it is on, follow `${CLAUDE_PLUGIN_ROOT}/skills/docs-sync/references/gh-issues.md`: a new task id gets an issue and the `Issue:` segment written back onto its metadata line, a ticked task gets its issue closed as completed, a rejected one closed as not planned with the dated reason as the comment, and a changed priority or cleared blocker gets its labels edited. Touch only the task you just worked on — closing an issue for a task somebody else ticked is what `project-docs:docs-sync` is for, and it looks at the whole plan before it acts. A GitHub failure is reported and the sequence continues: the spec, the ADR and the checkbox are the work, and losing them to a network error would be the expensive outcome.
7. **Stage and propose, do not commit.** `git add` everything, propose a commit message in English carrying the task id (`feat: WH-09 deduplicate webhook events by event id`), and stop. The user reviews the staged changes and approves the commit themselves. Documentation goes in the same commit as the code — that is what keeps the two from drifting.
