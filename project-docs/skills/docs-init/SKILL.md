...
Where the repository has a GitHub remote and `gh` is installed, add one sentence offering to mirror the plan to GitHub Issues with `project-docs:docs-sync`, saying how many issues that would create. Saying yes writes `docs/docs.config.json` and adds two lines to the tree you just wrote — the `Issue:` bullet in `plan/README.md` and a Conventions line in `docs/README.md` — which is `docs-sync`'s job rather than something to write ahead of an answer. One sentence, not a fifth question in the batch above: mirroring is worth offering once the plan exists and the number is real, and a project that says no keeps a tree with no `docs.config.json` in it, which is what every skill treats as "not mirrored".
